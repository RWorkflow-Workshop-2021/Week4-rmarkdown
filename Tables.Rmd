...
```{r}
kable(dt, align="lcccccc", caption = caption)
```
...
```{r}
kable_styling(
  kable(dt, align = "lcccccc", booktabs = TRUE, caption = caption)
  )
```

You can easily create some nice effects for your tables in html and pdf documents like tables that float to the side. But table caption seems to hang in this case.
